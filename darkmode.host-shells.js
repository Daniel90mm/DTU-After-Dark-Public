(function () {
    ...
    function fixCampusnetHeaderStyling() {
        ...
        // CampusNet's front-page widgets stop at Corbel/Verdana. Windows ships
        // those fonts; Linux commonly ships neither and otherwise falls all the
        // way back to the browser's default serif face.
        if ((window.location.pathname || '').toLowerCase() === '/cnnet/frontpage/dtu') {
            document.querySelectorAll('.widget, .widget__content').forEach(function (widgetPart) {
                if (!widgetPart || !widgetPart.style) return;
                widgetPart.style.setProperty('font-family', 'Corbel-Bold, Verdana, Arial, sans-serif', 'important');
            });
        }
        ...
        function applyCampusnetAccentElements() {
            ...
            document.querySelectorAll('.widget a[href], .widget__body a[href], .widget__content a[href]').forEach(function (a) {
                ...
                if (a.closest('table.gradesList tr.context_direct td:first-child, table.gradesList tr.context_alternating td:first-child')) return;
        ...
        if (breadcrumb && breadcrumb.style) {
            breadcrumb.style.setProperty('background', '#1a1a1a', 'important');
            breadcrumb.style.setProperty('background-color', '#1a1a1a', 'important');
        ...
        document.querySelectorAll('nav#breadcrumb.actualbreadcrumb a, nav#breadcrumb.actualbreadcrumb a.last').forEach(function (link) {
            if (!link || !link.style) return;
            link.style.setProperty('background', '#1a1a1a', 'important');
            link.style.setProperty('background-color', '#1a1a1a', 'important');
            link.style.setProperty('color', '#e0e0e0', 'important');
        });
