        ...
        function resolveMojanglesLogoElement(container) {
            ...
            if (slot && typeof slot.assignedElements === 'function') {
                ...
                for (var i = 0; i < assigned.length; i++) {
                    var el = assigned[i];
                    if (!el) continue;
                    if (el.matches && el.matches(logoSelector)) return el;
                    if (el.querySelector) {
                        var nested = el.querySelector(logoSelector);
                        if (nested) return nested;
                    }
                }
                for (var j = 0; j < assigned.length; j++) {
                    var assignedEl = assigned[j];
                    if (assignedEl && assignedEl.matches && assignedEl.matches(linkSelector)) return assignedEl;
                }
            }
